...
import io.blockchaingroup.vc.virtualcurrencytransfer.common.SentTransactionItem;
...
public class DaoService {
	...
	@Transactional
	public String doTransaction(String userId, List<RecipientAccount> recipientAccounts) {
	...
	public ReceivedTransaction getReceivedTransactions(String userId){

		Account receivedAccount = accountRepository.findByUserId(userId);

		if (receivedAccount == null)
			throw new AccountNotFoundException("There is no Account with id: "+ userId);

		List<TransactionRecipient> userReceivedTransactions = transactionRecipientRepository.findByUserId(userId);

		ReceivedTransaction receivedTransactions = new ReceivedTransaction();

		List<SentTransactionItem> senders = new ArrayList<>();

		double totalReceivedAmount = 0;

		for (TransactionRecipient transactionRecipient : userReceivedTransactions) {

			totalReceivedAmount+=transactionRecipient.getAmount();

			Transaction receivedTransaction = transactionRecipient.getTransaction();
			Account senderAccount = receivedTransaction.getSender();


			SentTransactionItem sender = new SentTransactionItem();

			sender.setUserName(senderAccount.getUserName());
			sender.setUserId(senderAccount.getUserId());
			sender.setEmail(senderAccount.getEmail());
			sender.setAmount(transactionRecipient.getAmount());
			sender.setTransactionDate(receivedTransaction.getTransactionDate());
			senders.add(sender);

		}

		receivedTransactions.setAmount(totalReceivedAmount);
		receivedTransactions.setSenders(senders);
